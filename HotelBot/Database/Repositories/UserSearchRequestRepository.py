...
class UserSearchRequestRepository:
     ...
     @staticmethod
     def create(city, guests, rooms, startDate, endDate, priceRange, userId):
        handle = DbHandle.get();
        try:
            user = UserRepository.getById(userId);
            request = DbUserSearchRequest(city=city, priceRange=priceRange, guests=guests, rooms=rooms, user=user.id, startDate=startDate, endDate=endDate);
            request.save();
        except Exception as e:
            getLogger().fatal(e, exc_info=True);
        finally:
